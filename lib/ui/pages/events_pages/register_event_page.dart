...
class RegisterEventPage extends StatelessWidget {
  ...
  const RegisterEventPage({Key key, this.eventData}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return FocusWidget(
      child: Scaffold(
        body: BaseWidget<RegisterEventPageModel>(
            model: RegisterEventPageModel(
                api: Provider.of(context), auth: Provider.of(context), eventData: eventData),
  ...
  Widget privacyText(RegisterEventPageModel model) {
    return Row(
  ...
  Container buildSignupForm(RegisterEventPageModel model, BuildContext context) {
    return Container(
      margin: EdgeInsets.symmetric(
          vertical: ScreenUtil.screenHeightDp * 0.02, horizontal: ScreenUtil.screenWidthDp * .06),
      decoration: BoxDecoration(
  ...
  buildProceedButton(RegisterEventPageModel model, BuildContext context) {
    final locale = AppLocalizations.of(context);
  ...
  buildSucessAlert(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Color(0xFFFFC100), borderRadius: BorderRadius.circular(25)),
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              AppLocalizations.of(context).get('You have successfully registered for this event'),
              maxLines: 3,
              style: TextStyle(
                  color: AppColors.primaryElement, fontSize: 18, fontWeight: FontWeight.bold),
            ),
          )
        ],
      ),
...
class RegisterEventPageModel extends BaseNotifier with Validator {
  final HttpApi api;
  ...
  RegisterEventPageModel({this.eventData, this.api, this.auth}) {
  ...
  onSubmit(BuildContext context) async {
    ...
    if (formKey.currentState.validate() && agree) {
      try {
        registered = await api.registerEvent(
            eventId: eventData.id, email: email.text, mobile: mobile.text, name: fullName.text);
